...
void step_init()
{
  ...
  double _speed_rate=RK.readDouble(speed_rate);
  if(_speed_rate==0)
    _speed_rate=1;
  a1 = 800*_speed_rate;
  a2 = 51200*_speed_rate;   // pulse/s^2
  a3 = 51200*_speed_rate;   // pulse/s^2
  

  min_hiz = 64.0*_speed_rate;  // pulse/s
  mid_hiz = 320*_speed_rate;   // pulse/s
  max_hiz = 6400*_speed_rate;  // pulse/s
  home_hiz = 2000*_speed_rate; // pulse/s
  home_hiz_yaklasma = 64.0*_speed_rate;
...
void IRAM_ATTR step_pulse()
{
  ...
  if ((micros_t - step1_high_t) >= step1_interval)
  {
    ...
    { // iniş rampası eğim a3 en düşük hızı min_hiz
      ...
      if (hiz1 < min_hiz)
      {
        ...
        if (dir1 < 0)
          digitalWrite(M1_DIR_PIN, HIGH); // eksi yön dir=HIGH
        else
          digitalWrite(M1_DIR_PIN, LOW); // artı yön dir=LOW
      }
    }
    ...
    if (step1_calisma_modu != 0)
    {
      if (systemStatus.m1.limit_ok&&systemStatus.m1.soft_limit_ok)
      {
  ...
  if ((micros_t - step2_high_t) >= step2_interval)
  {
    ...
    if (step2_calisma_modu != 0)
    {
      ...
      if (systemStatus.m2.limit_ok&&systemStatus.m2.soft_limit_ok)
      {
